    ...
    if iyear <= 2020
        for ispc = 3
            for years = iyear
                ...
                emis_daily = zeros(nx,ny,ndays+31,'single');
                ...
                filename_neyr = [location,prefix,num2str(years+1),'_01_',char(spc_name(ispc)),postfix];
                emis_daily(:,:,ndays + 1:end) = ncread(filename_neyr,char(spc_name(ispc)));
        ...
        filepath1 = ['F:\stilt_output\GDAS05\footprints_',num2str(iyear),'_gfs_nohnf_receptor_se\'];
        load([filepath1,'footprints_daily.mat']);
        footprints_exp1 = footprints_exp;
        filepath2 = ['F:\stilt_output\GDAS05\footprints_',num2str(iyear+1),'01_gfs_nohnf_receptor_se\'];
        load([filepath2,'footprints_daily.mat']);
        footprints_exp2 = footprints_exp;
        clearvars footprints_exp;
        ...
        for ii = 1:length(percent_N)
            ...
            if sum_percent > 0.2  % 10% percentile
                index_edge = ii + 1;
                break;
            end
        end
        ...
        eval(['pm25obs_yr = pm25_daily_',site_name_tmp,'(startday:endday);']);
        load(['E:\work_for_wildfires\data\air quality\mat_data\',site_name_tmp,'_pm25_',num2str(iyear+1),'0101_',num2str(iyear+1),'1231.mat']);
        eval(['pm25obs_neyr = pm25_daily_',site_name_tmp,'(1:31);']);
        pm25obs = cat(1,pm25obs_yr,pm25obs_neyr);
...
%         figure1 = figure;
%         % plot(pm25_daily_footscray(213:end));
%         plot(pm25obs);
%         hold on;
%         bar(mark_day*100);
    else
